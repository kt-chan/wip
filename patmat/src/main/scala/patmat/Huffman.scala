...
object Huffman {
  ...
  // Part 1: Basics 
  // tree match ... which returns the total weight of a given Huffman tree.
  ...
  // tree match ... chars which returns the list of characters defined in a given Huffman tree
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    ...
    def index(target: Char, chars: List[(Char, Int)]): Int = {
      var i = 0
      var s = -1
      chars.foreach(pair => {
        if (pair._1 == target) { s = i; return s; }
        i = i + 1
      })
      return s;
    }
   ...
   */
  def singleton(trees: List[CodeTree]): Boolean = trees.nonEmpty && trees.length == 1
   ...
   */
  def until[T](check: List[T] => Boolean, transform: List[T] => List[T])(trees: List[T]): T = {

    if (check(trees) == false) {
      until(check, transform)(transform(trees))
    } else
      return trees.head;
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    ...
    def decodeInner(subTree: CodeTree, subBits: List[Bit], accu: List[Char]): List[Char] = subBits match {
      ...
      case h :: tail => subTree match {
        ...
        case l: Leaf => decodeInner(tree, subBits, accu :+ l.char)
        case _ => throw new Exception("Class is not defined exception.")
      }
      //Error Exception
      case _ => throw new Exception("Error: logic exception.")
    }

    decodeInner(tree, bits, List())
  }
  ...
  def convert(tree: CodeTree): CodeTable = {
    ...
    def convertInner(tree: CodeTree, result: List[Bit]): CodeTable = tree match {
      case f: Fork => mergeCodeTables(convertInner(f.left, result :+ 0), convertInner(f.right, result :+ 1))
      case l: Leaf => List((l.char, result))
      case _ => throw new Exception("Class is not defined exception.")
    }
    convertInner(tree, List())
  }
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    var s = List[Bit]()
    text.foreach { c => s = s ::: codeBits(myCodeTable)(c) }
    println("result =" + s)
    return s
  }
}
