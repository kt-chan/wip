...
class HuffmanSuite extends FunSuite {
  ...
  test("decode and encode a long text should be identity") {
    new TestTrees {
      val myCodeTree = createCodeTree(string2Chars("aabcccca"))
      val encoded_a = encode(myCodeTree)("a".toList)
      val encoded_b = encode(myCodeTree)("b".toList)
      val encoded_ab = encode(myCodeTree)("ab".toList)
      println("a: " + encoded_a)
      println("b: " + encoded_b)
      println("ab: " + encoded_ab)
      assert(decode(myCodeTree, encoded_a) === "a".toList)
      assert(decode(myCodeTree, encoded_b) === "b".toList)
      assert(decode(myCodeTree, encoded_ab) === "ab".toList)
    }
  }
  test("Quick Code") {
